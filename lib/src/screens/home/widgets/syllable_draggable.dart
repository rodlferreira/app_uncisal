...
class SyllableDraggable extends StatelessWidget {
  ...
  const SyllableDraggable({Key key, @required this.syllable, this.dropped = false}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    if (dropped) {
      return SyllableButton(syllable: syllable, show: false);
    }
    final Widget btn = SyllableButton(syllable: syllable);
    return Draggable<Syllable>(
      data: syllable,
      child: btn,
      childWhenDragging: SyllableButton(syllable: syllable, show: false),
      feedback: Material(
